...
# chroot ${HOST} atomic install fedora-rdo-rabbitmq-atomic
chroot ${HOST} docker run -d --name rabbitmq -p 5672:5672 --env-file=/etc/openstack.env imain/fedora-rdo-rabbitmq
...
chroot ${HOST} docker run --name glance-registry -p 9191:9191 -d \
       --env-file=/etc/openstack.env rthallisey/fedora-rdo-glance-registry:latest

chroot ${HOST} docker run --name glance-api -d -p 9292:9292 \
...
chroot ${HOST} docker run -d --privileged \
       -v /sys/fs/cgroup:/sys/fs/cgroup \
       -v /var/lib/nova:/var/lib/nova \
...
chroot ${HOST} docker run --name nova-network -d --privileged \
       --net=host \
